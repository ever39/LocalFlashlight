...
namespace localFlashlight
{
    internal class LightScript : MonoBehaviour
    {
        ...
        //debugging values, in case more errors pop up
        ...
        private void Awake()
        {
            Plugin.mls.LogInfo("LocalFlashlight mod script is awake, making light");
            isLightLoaded = false;
            hasShownError = false;
            errorCount = 0;
        }
        ...
        public void Update()
        {
            ...
            {
                if (!hasShownError && errorCount >= 5)
                {
                    Plugin.mls.LogError($"error popped up while the mod was doing something\n as far as i'm concerned, this only ever pops up if you have the \"foggy screen\"\n alternatively, there could be some other mods conflicting with this one\n HOWEVER if it isn't from another mod, or the mod doesn't work as intended, please report the issue on the mod's github page (alongside a screenshot and the mods you're using)\n thanks, and sorry for the big error text\n{e}");
                    hasShownError = Plugin.onlyShowErrorOnce.Value;
                }
                errorCount++;
                return;
            }
        ...
        //Handles setting the mod's configs when trying to find the player
        ...
        private void UpdateModValues()
        {
            ...
            //the globalFlashlightInterferenceLevel integer is thankfully static, so we can use that to change the light's intensity when a ghost girl haunts the player
            if (FlashlightItem.globalFlashlightInterferenceLevel >= 1)
            {
                locallight.intensity = Plugin.Intensity.Value * Patches.randomLightInterferenceMultiplier;
                Plugin.mls.LogDebug("flickering flashlight (ghost girl)");
                return;
            }
        ...
        {
            Plugin.mls.LogDebug("Flickered localFlashlight (battery ran out)");
            regenCool = 0.3f + (Plugin.BatteryBurnOut.Value ? burnOutCooldown : batteryCooldown);
            flashState = false;
            PlayNoise(nochargetoggle, 1);
            locallight.enabled = false;
