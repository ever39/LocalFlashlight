...
namespace localFlashlight
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            rechargeOption = Config.Bind<RechargeOptions>("Other", "(EXPERIMENTAL) Recharge method", RechargeOptions.Time, "The way that the flashlight can be recharged. Time, Shake and Dynamo are intended for shorter battery times while Apparatice and OnShipEnter are intended for longer battery times");
            ...
            if (bundle == null)
            {
                mls.LogWarning("failed to get mod assets, sounds won't be heard, HUD won't be there, and you'll probably get a ton of errors, if this keeps happening, reinstall the mod");
            }

            ///detetcting scene, and adding the gameobject if the scene is the in-game ship scene
            SceneManager.sceneLoaded += OnPlayingSceneLoaded;

            ///patching hud (don't wanna implement it in the lightscript as i still have to patch stuff other than the hud anyway)
        ...
        {
            //detect if in-game (somehow works with latecompany's mid-round join as you still enter the SampleSceneRelay when you're loading the seed)
