...
namespace localFlashlight
{
    internal class Patches
    {
        ...
        public static void UpdateBatteryInfo()
        {
            ...
            UIContainer.SetActive(LightScript.isLightLoaded);
            ...
            ///The disabled style code. Yes, this is all of it.
            if (selectedStyle == BatteryDisplayOptions.Disabled)
            {
        ...
        #region Code related to the Apparatice recharge method
        [HarmonyPatch(typeof(HUDManager), "RadiationWarningHUD")]
        [HarmonyPostfix]
        public static void DisableHUDOnAppDisconnect()
        {
        ...
        [HarmonyPostfix]
        public static void ReEnableHUD()
        {
            if (selectedRecharge == RechargeOptions.Apparatice) UIContainer.SetActive(false);
            isAppTaken = false;
            Plugin.mls.LogDebug("ship left, redisabling hud and light is back to infinite battery");
        }
        #endregion
        ...
        {
            if (UIContainer.GetComponent<CanvasGroup>().alpha < 1) fadeIn = true;
        }

        private static void HideUI()
        {
            if (UIContainer.GetComponent<CanvasGroup>().alpha >= Plugin.UIHiddenAlpha.Value) fadeOut = true;
        }
        #endregion
    }
}
